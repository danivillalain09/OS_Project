...
class Boats:
    def __init__(self, name, port_object, sql_connection, starting_time, priority, delay):
        ...
        self.disable_delay = delay  # Disable delay
        self.disable_priority = priority  # Disable priority
...
print("----------------------------------------------------------------------")
text = "WELCOME TO THE PORT SIMULATION"
print(f"{Fore.LIGHTCYAN_EX}{text.center(60)}{Style.RESET_ALL}")
print("----------------------------------------------------------------------")
hakibi = str(input("Press <ENTER> to start the simulation: "))

print("----------------------------------------------------------------------")
print("First, we will create the port. Please, insert the following information:")
number_of_boats = int(input("- How many boats do you want to simulate? "))
number_of_docks = int(input("- Hoy many loading areas do you want to simulate? "))
number_of_cranes = int(input("- How many cranes do you want to simulate? "))
number_of_transporters = int(input("- How many transporters do you want to simulate? "))
if number_of_cranes == 0:
    number_of_cranes = 1
if number_of_transporters == 0:
...
if hakibi == "123456":
    ...
    password = str(input("In order to see the results, what is your SQL password? "))

sql = SQLConnection()
start = time.time()
starting_number = sql.get_starting_number()
control = Control(number_of_boats, number_of_docks, sql, start)
sql.add_columns(Boats(1, control, sql, 0, False, False), Container(0, 0, "E", "S", "C"), Worker(0, control, "Crane"), Crane("Crane"))
print("----------------------------------------------------------------------")
disable_delay = False
disable_priority = False
print("There are two functions that you can disable in order to see the results of the simulation.")
print("1. The first one is the delay in arriving.\n "
      "   This function will delay the arrival of the boats.")
print("2. The second one is the boat priority.\n"
      "   This function will only let the boats enter if they were the first to get in the queue.")
print()

disable = str(input("- Do you want to disable any of them? (Yes / No) ")).lower()

if disable == "yes":
    disable = str(input("- Which one do you want to disable? (1 / 2 / Both) ")).lower()
    if disable == "1":
        disable_delay = True
    elif disable == "2":
        disable_priority = True
    elif disable == "both":
        print("Disabling both functions...")
        disable_delay = True
        disable_priority = True
    else:
...
with concurrent.futures.ThreadPoolExecutor(max_workers=number_of_cranes) as executor:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=number_of_transporters) as executor:
        worker_list = [Worker(i, control, "Transporter") for i in range(number_of_cranes + 1, number_of_workers + 1)]
        for index, worker_list in enumerate(worker_list):
            executor.submit(worker_list.simulation)
            time.sleep(1)
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=int(number_of_threads)) as executor:
                boat_list = [Boats(i, control, sql, start, disable_priority, disable_delay) for i in
